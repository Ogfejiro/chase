...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 ${scrolled ? "bg-[rgba(3,8,16,0.92)] backdrop-blur-xl border-b border-[rgba(0,180,255,0.20)]" : "bg-transparent"}`}>
        ...
        <div className="hidden md:flex items-center gap-8">
          {["About", "Projects", "Network", "Contact"].map((item) => (
            <a key={item} href={`#${item.toLowerCase()}`}
              ...
              className="text-xs text-[rgba(242,237,230,0.95)] hover:text-[#00b4ff] transition-colors duration-200 tracking-widest uppercase no-underline">
              {item}
            </a>
          ))}
          <a href="#contact"
      ...
      {/* Mobile menu */}
      <div className={`md:hidden border-t border-[rgba(0,180,255,0.20)] bg-[rgba(3,8,16,0.97)] transition-all duration-300 overflow-hidden ${menuOpen ? "max-h-64 opacity-100" : "max-h-0 opacity-0"}`}>
        <div className="px-6 py-6 flex flex-col gap-5">
          {["About", "Projects", "Network", "Contact"].map((item) => (
            <a key={item} href={`#${item.toLowerCase()}`} onClick={() => setMenuOpen(false)}
              style={{ fontFamily: "var(--font-mono,'Space Mono',monospace)" }}
              className="text-sm text-[rgba(242,237,230,0.95)] hover:text-[#00b4ff] transition-colors uppercase tracking-widest no-underline">
              {item}
            </a>
          ))}
        </div>
