...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      {/* Corner accents */}
      <div className="absolute top-16 left-0 w-16 h-16 md:w-24 md:h-24 border-l border-t border-[rgba(0,180,255,0.3)]" />
      <div className="absolute top-16 right-0 w-16 h-16 md:w-24 md:h-24 border-r border-t border-[rgba(0,180,255,0.3)]" />
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-5 md:px-10 pt-28 md:pt-32 pb-20">
        ...
        <div className="flex items-center gap-3 mb-5 md:mb-8" data-aos="fade-up" data-aos-delay="100" data-aos-duration="800">
          <div className="h-px w-10 md:w-20 flex-shrink-0" style={{ background: "linear-gradient(to right, #00b4ff, transparent)" }} />
          <span
            style={{ fontFamily: "var(--font-mono,'Space Mono',monospace)", letterSpacing: "0.15em" }}
            className={`text-xs md:text-base text-[#00b4ff] uppercase transition-opacity ${scrambling ? "opacity-60" : "opacity-100"}`}
          >
            {displayRole}
          </span>
          <div className="h-px w-10 md:w-20 flex-shrink-0" style={{ background: "linear-gradient(to left, #00b4ff, transparent)" }} />
        </div>
        ...
        <div className="flex flex-wrap gap-3 md:gap-4 mb-10 md:mb-16" data-aos="fade-up" data-aos-delay="300" data-aos-duration="800">
          ...
          <a
            ...
            className="hover:border-[#00b4ff] hover:text-[#00b4ff] transition-all duration-300 no-underline"
            style={{ fontFamily: "var(--font-mono,'Space Mono',monospace)", fontSize: "11px", letterSpacing: "0.1em", textTransform: "uppercase", padding: "12px 28px", border: "1px solid rgba(0,180,255,0.45)", color: "white", display: "inline-block" }}
          >
            Follow on X →
          </a>
        </div>
        ...
        <div
          ...
          style={{ border: "1px solid rgba(0,180,255,0.25)" }}
          data-aos="fade-up" data-aos-delay="400" data-aos-duration="800"
        >
          {STATS.map((stat, i) => (
            <div
              key={i}
              className="group hover:bg-[rgba(0,180,255,0.06)] transition-colors duration-200"
              style={{ padding: "16px 20px", borderRight: i < 3 ? "1px solid rgba(0,180,255,0.25)" : "none", display: "flex", flexDirection: "column", gap: "4px" }}
            >
              <span
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 z-10">
        <span style={{ fontFamily: "var(--font-mono,'Space Mono',monospace)", fontSize: "9px" }} className="text-[rgba(0,180,255,0.6)] tracking-widest uppercase">Scroll</span>
        <div className="w-px h-8 md:h-12 animate-pulse" style={{ background: "linear-gradient(to bottom, rgba(0,180,255,0.5), transparent)" }} />
      </div>
    </section>
  );
}